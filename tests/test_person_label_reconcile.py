...
def test_search_labels_dedupes_display_name_and_normalizes():
    person = Person(display_name="정이한", aliases_json=["이한", "정이한", "  내아들 ", "이한"])
    assert person.search_labels() == ["정이한", "이한", "내아들"]
...
def test_person_label_values_union_across_people():
    a = Person(display_name="정이한", aliases_json=["이한"])
    b = Person(display_name="person-000600", aliases_json=[])
    assert person_label_values([a, b]) == ["정이한", "이한", "person-000600"]


def test_finder_flags_files_missing_alias_and_reconcile_backfills():
    factory = _session_factory()
    with factory() as session:
        named = Person(id=40, display_name="정이한", aliases_json=["이한", "내아들"])
...
def test_reconcile_unions_multiple_people_and_drops_stale_tags():
    factory = _session_factory()
    with factory() as session:
        a = Person(id=40, display_name="정이한", aliases_json=["이한"])
        b = Person(id=41, display_name="장윤겸", aliases_json=["윤겸"])
        session.add_all([a, b])
