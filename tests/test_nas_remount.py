...
//dongeui@DS920._smb._tcp.local/homes on /Volumes/homes (smbfs, nodev, nosuid, mounted by dongeui)
//guest@OtherNAS/media on /Volumes/media (smbfs, nodev, nosuid, mounted by dongeui)
"""
...
def test_parse_network_share_url_matches_mountpoint_and_subpaths() -> None:
    assert (
        ...
        == "smb://dongeui@DS920._smb._tcp.local/homes"
    )
    assert (
        parse_network_share_url(MOUNT_OUTPUT, "/Volumes/homes/dejeong/Photos")
        == "smb://dongeui@DS920._smb._tcp.local/homes"
    )
    assert parse_network_share_url(MOUNT_OUTPUT, "/Volumes/media") == "smb://guest@OtherNAS/media"
    # 로컬 디스크 경로는 매칭되지 않는다.
    assert parse_network_share_url(MOUNT_OUTPUT, "/Users/dongeui/Pictures") is None
...
def test_record_and_remount_roundtrip(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert remounter.known_urls() == {"/Volumes/homes": "smb://dongeui@DS920._smb._tcp.local/homes"}
    ...
    assert mounted == ["smb://dongeui@DS920._smb._tcp.local/homes"]
