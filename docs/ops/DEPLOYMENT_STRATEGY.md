...
2. LAN / 네트워크 접근
   - Docker/서버 배포 경로에서 제공 (`0.0.0.0` 바인딩 + admin token 가드)
   - Mac 앱에는 LAN 공유 토글이 없다 — 항상 local-only (기능 제거됨)
   - 같은 네트워크의 다른 기기에서 접근 (Docker/서버)
...
2. bundled backend runtime
   ...
   - Mac 앱은 항상 `127.0.0.1`에 바인딩 (LAN 공유 제거)
   - Docker/서버 배포에서만 `0.0.0.0` 바인딩 + admin token 가드
...
4. web UI
   ...
   - 기본 브라우저 런처로 표시 (창 없는 메뉴바 앱 — 앱 내장 WebView 창 없음)
...
4. Mac 앱은 local-only, LAN/네트워크 접근은 Docker/서버 배포 경로로 제공
