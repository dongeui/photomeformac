# Photome Mac 앱 백엔드 런타임 계약

## 목적

Mac 앱은 Docker 없이 Photome 백엔드를 로컬 프로세스로 실행한다. 이 문서는 macOS 앱 shell이 Python/FastAPI 백엔드를 시작, 감시, 종료할 때 지켜야 할 계약이다.
...
1. 기본값은 local-only다.
2. `PHOTOME_SERVER_HOST=127.0.0.1`을 사용한다.
3. LAN 공유는 사용자가 명시적으로 켠 경우에만 `PHOTOME_SERVER_HOST=0.0.0.0` 또는 선택 인터페이스로 바꾼다.
...
PHOTOME_DATA_ROOT=<AppData>/data
PHOTOME_DERIVED_ROOT=<AppData>/derived
PHOTOME_MODEL_ROOT=<AppData>/models
PHOTOME_GEODATA_ROOT=<AppData>/models/geodata
PHOTOME_DATABASE_PATH=<AppData>/data/photome.sqlite3
PHOTOME_OFFLINE_MODE=1
PHOTOME_CLIP_ENABLED=1
PHOTOME_SCHEDULER_ENABLED=0
PHOTOME_SEMANTIC_SCHEDULER_ENABLED=1
```

`PHOTOME_SOURCE_ROOTS`는 사용자가 선택한 macOS 원본 폴더 목록을 콤마로 연결한다. Docker 호환용 `/photos` 경로로 바꾸지 않는다.
...
stdout/stderr는 앱 로그로 라우팅한다. 패키징 후에는 `~/Library/Logs/Photome/`에 저장하는 것을 기본으로 한다.

## AI/오프라인 정책

1. 앱은 CLIP 모델 없이도 실행되어야 한다.
2. `PHOTOME_OFFLINE_MODE=1`에서는 자동 다운로드를 시도하지 않는다.
