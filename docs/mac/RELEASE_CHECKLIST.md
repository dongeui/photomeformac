...
## 8. LAN admin 가드 (Docker/서버 배포 한정)

> Mac 앱은 LAN 공유를 제거(local-only 고정)했다. 아래는 백엔드를 `0.0.0.0`으로 노출하는 Docker/서버 배포에만 해당한다.

백엔드를 `0.0.0.0`에 바인딩하면 원격 기기의 조회성 화면은 열 수 있지만, 스캔/사람 관리/검색 가중치/원본 다운로드 같은 관리자성 API는 `X-Trove-Admin-Token`(`TROVE_LAN_ADMIN_TOKEN`) 보호를 받는다.

확인할 점:

- 같은 호스트의 localhost 요청은 토큰 없이 계속 동작
- 다른 기기의 관리자 API 호출은 토큰 없으면 401
- 토큰은 배포 환경 설정으로만 주입하고 저장소에는 커밋하지 않음
