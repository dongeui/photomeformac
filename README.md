...
**사진첩에 보이는 사진은 분석(검색 색인)이 끝난 것이다.** 즉 "보이면 검색된다" — 분석이 끝나기 전 사진은 사진첩에 노출하지 않고, 대신 "분석 중 N장 · 완료되면 표시" 안내를 보여준다. 완료되면 자동으로 사진첩에 나타난다. ("보이는데 검색은 안 됨"을 구조적으로 막는다 — [docs/engineering/FINDABILITY_CONTRACT.md](docs/engineering/FINDABILITY_CONTRACT.md).)
...
> 이 동작은 `TROVE_GALLERY_REQUIRE_ANALYSIS_COMPLETE=0`으로 끌 수 있다(끄면 분석 전이라도 즉시 노출, 단 잠깐 검색에 안 잡힐 수 있음).

### 인물 검색 보강 (수동 태깅)

얼굴이 자동 검출되지 않은 사진(아기·옆모습·HEIC 등)은 사진첩 라이트박스의 **"인물"** 영역에서 직접 이름을 달 수 있다 — 이름만 달면 그 사진이 이름 검색에 잡히고, **"얼굴 지정"으로 얼굴 영역을 드래그**하면 그 얼굴로 학습까지 해서 이후 같은 사람의 새 사진 자동 매칭이 좋아진다.
...
© 2026 oovelo. 독점 소프트웨어 — 소스는 투명성·평가 목적으로만 공개한다.
사용·복제·수정·재배포는 oovelo의 별도 서면 동의 없이는 허용되지 않는다.
전문은 [LICENSE](LICENSE) 참고. (번들된 서드파티 구성요소는 각자의 라이선스를 따른다.)
문의: https://oovelo.com
