...
Trove는 **창 없는 메뉴바 앱**이다. 사진첩/검색/설정은 메뉴의 "사진첩 열기"·"설정 열기"로 기본 브라우저에서 열고, 네이티브 조작(폴더 선택·재시작·자동 시작)만 메뉴바 아이콘에 모여 있다. 앱을 켜면 선택해 둔 사진 폴더가 있을 경우 백엔드(FastAPI + 이미지 AI)가 자동으로 함께 뜬다.
...
동기화가 도는 동안에는 백엔드를 내려 작업을 끊는 조작("사진 폴더 선택", "다시 시작")이 잠긴다. "사진첩 열기"·"설정 열기"는 동기화 중에도 열 수 있다. 백엔드가 응답을 멈추면 상태가 "오류"로 바뀌고 "다시 시작"이 복구용으로 다시 활성화된다.
