"""Caption provider registry — select provider via TROVE_CAPTION_PROVIDER env var."""
...
def get_caption_provider() -> CaptionProvider | None:
    ...
    TROVE_CAPTION_PROVIDER:
      - 'moondream' (default when set): use Moondream2 local model
      - unset / 'none': caption generation disabled
    """
    if os.environ.get("TROVE_OFFLINE_MODE", "").strip().lower() in {"1", "true", "yes", "on"}:
        logger.info("caption provider disabled in offline mode")
        return None

    provider_name = os.environ.get("TROVE_CAPTION_PROVIDER", "").strip().lower()
